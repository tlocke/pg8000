...
class Tests(unittest.TestCase):
    ...
    def testCopyToWithQuery(self):
        with closing(db.cursor()) as cursor:
            stream = StringIO()
            cursor.copy_to(
                stream, query="COPY (SELECT 1 as One, 2 as Two) TO "
                "STDOUT WITH DELIMITER 'X' CSV HEADER QUOTE AS 'Y' "
                "FORCE QUOTE Two")
    ...
    def testCopyFromWithTable(self):
        with closing(db.cursor()) as cursor:
            ...
            self.assertEquals(
                retval, [(1, 1, u'1'), (2, 2, u'2'), (3, 3, u'3')])

    def testCopyFromWithQuery(self):
        with closing(db.cursor()) as cursor:
            stream = StringIO("f1Xf2\n1XY1Y\n")
            cursor.copy_from(
                stream, query="COPY t1 (f1, f2) FROM STDIN WITH DELIMITER 'X' "
                "CSV HEADER QUOTE AS 'Y' FORCE NOT NULL f1")
            self.assert_(cursor.rowcount == 1)

            cursor.execute("SELECT * FROM t1 ORDER BY f1")
            retval = cursor.fetchall()
            self.assertEquals(retval, [(1, 1, None)])
