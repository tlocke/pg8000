...
versions 2.7+ and 3.5+, on CPython and PyPy, and PostgreSQL versions 9.5+.
...
==== pg8000.connect(user, host='localhost', unix_sock=None, port=5432, database=None, password=None, ssl=None, timeout=None, application_name=None)
...
ssl::
  ...
  function. The default for `ssl` is `None`.
...
This will run the tests against all the installed Python versions, and the
installed PostgreSQL version listening on port 5432.
