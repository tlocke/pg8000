...
==== pg8000.connect(user, host='localhost', source_address=None, unix_sock=None, port=5432, password=None, ssl_context=None, timeout=None, tcp_keepalive=True,
\*\*init_params)
...
init_params::
  ...
  If your server character encoding is not '`ascii`' or '`utf8`', then
  you need to provide values as bytes, eg. `'my_db'.encode('EUC-JP')`.
